...
- **Billing & Fees (관리비 부과/수납):** Proportional fee calculation based on exclusive area, per-unit billing table with payment status (수납/미납), 12-month trend chart (Recharts), interim settlement for move-in/move-out (일할 계산), Kakao notification simulation for billing notices (고지서).
