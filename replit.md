...
- **의사결정 질문 형식(체크박스 표기) 원칙**: 사용자에게 결정·선택을 요청할 때는 항상 항목별 체크박스 형식으로 제시한다. 단일 선택이든 복수 선택이든 동일하게 적용한다. 형식 예시:
  - `- [ ] 옵션 A — 한 줄 설명 (영향/리스크)`
  - `- [ ] 옵션 B — 한 줄 설명 (영향/리스크)`
  - `- [ ] 옵션 C — 한 줄 설명 (영향/리스크)`
  필요 시 그룹별로 소제목을 두고 각 그룹 아래 체크박스를 나열한다. 권장안에는 `(권장)` 표시를 붙인다. 자유 서술형 답변이 필요한 경우에도 우선 체크박스 옵션을 제시한 뒤 "기타(직접 입력)" 항목을 마지막에 둔다.
