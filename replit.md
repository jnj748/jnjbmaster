...
- **Dashboard Restructuring:** Bottom nav uses 홈/일정/회계/시설/업무. Desktop sidebar organized with section headers (관리비회계, 시설관리, 입주/자산관리, 보고/서식). StatCards are clickable with `href` navigation. Dashboard shows building name from BuildingContext when registered. Sidebar top "건물 정보" links to `/building-info` (read-only dashboard with building overview, inspection/maintenance/occupancy stats). Settings gear icon at sidebar bottom links to `/settings` (tabs: 내정보수정 + 건물정보수정). Old `/building-setup` route still exists but is accessed through settings tab.
...
- **Partner Marketplace Enhancement:** Extended vendor categories with waterproofing (방수), maintenance_repair (영선/수선유지), defect_diagnosis (하자진단), building_maintenance (건물관리), mechanical (기계설비). Vendors support `subCategories` field for granular specialization. Buildings have `approvalDate` for warranty tracking.
- **Warranty Tracking (하자담보):** `warranty_presets` table (12 presets covering waterproofing, exterior, painting, tiling, window, plumbing, electrical, elevator, firefighting, HVAC, landscaping, road/pavement with Korean legal basis). `building_warranties` table links presets to buildings with start/expiry dates, contractor info, and 60/30-day alert tracking. Full CRUD API at `/warranties/*`. Dashboard alerts include `warranty_expiry` type.
- **Seasonal Maintenance Workflows:** `seasonal_maintenance_presets` table (16 presets across 12 months). Dashboard endpoint `/dashboard/seasonal-suggestions` returns current month's suggestions with priority levels and RFQ category mapping. Dashboard UI shows green-bordered suggestion cards with one-click RFQ creation.
...
- **Building Setup & Integration:** Includes a `buildings` table with extended fields (landArea, buildingArea, buildingCoverageRatio, floorAreaRatio), links users to buildings, and integrates with the `건축물대장` (Building Register) API for building information and automated inspection scheduling based on legal criteria. Step 0 uses Kakao Postcode address search (free, no API key) to auto-fill address fields and derive building register codes (시군구코드, 법정동코드, 본번, 부번) from the selected address's bcode and jibun. After address selection, 총괄표제부 (getBrRecapTitleInfo) + 표제부 (getBrTitleInfo) are fetched in parallel via Promise.allSettled for partial failure resilience.
