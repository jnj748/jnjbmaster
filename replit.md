...
- **Delinquency Automation (연체 자동화):** Automated detection of 2+ month overdue units via scheduler. `delinquency_actions` table tracks per-unit delinquency status (detected→notice_sent→parking_suspended→resolved). API: GET /delinquency (list), GET /delinquency/summary, POST /delinquency/:id/notify (simulated dunning), POST /delinquency/:id/suspend-parking (suspends vehicle status + history), POST /delinquency/:id/resolve (restores vehicles). Dashboard widgets on both main dashboard (summary card) and accounting-dashboard (full management list with action buttons). Monthly dedup prevents re-detection.
