...
- **Accounting→Report Pipeline (회계→보고 파이프라인):** Monthly report auto-generation (scheduler + manual POST /monthly-summary-reports) queries monthlyPaymentsTable to append "■ 회계 현황" section with: 부과 총액, 수납 총액, 수납률, 미납 세대 수, 전월 대비 증감 (▲/▼/→ with % change). Accountant dashboard has hardcoded recurring tax deadlines (매월 5일 세무사 마감, 10일 원천세/4대보험/세금계산서, 25일 관리비 고지서, 28일 세무사 자료 요청; 분기말 부가세 신고) with D-3 warning badges (red highlight + pulse animation). HQ dashboard monthly reports show expandable accounting data (totalBilled, totalCollected, collectionRate, unpaidUnits, momChange).
