...
- **소모품 공급 (MRO) 개발 방향 (Phase 3 예정)**: 소모품(형광등·화장지·세제·필터 등)은 MRO 쇼핑몰 형태로 별도 개발 예정. 재고·물류·정산 인프라 필요로 견적 BM과 분리. 현재 `consumables` 카테고리는 `active=false` 유지. Phase 1 견적 크레딧 BM 검증 후 진행.
