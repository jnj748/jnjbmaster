...
export function FirstEntryConsentModal() {
  ...
  const isPartner = user?.role === "partner" || user?.portalType === "partner";
  ...
  useEffect(() => {
    ...
    (async () => {
      const checks = await Promise.all([
        checkConsent(token, "intermediary_terms"),
        checkConsent(token, "privacy_policy"),
        ...(isPartner ? [checkConsent(token, "partner_terms")] : []),
      ]);
      const allOk = checks.every(Boolean);
      if (!cancelled && !allOk) setOpen(true);
    })();
    return () => {
      cancelled = true;
    };
  }, [user, token, isPartner]);

  if (!user || !token) return null;

  const allAgreed = agreeTerms && agreePrivacy && (!isPartner || agreePartner);
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 text-sm">
          ...
          <ScrollArea className="h-48 rounded-md border p-3">
            <pre className="text-[11px] whitespace-pre-wrap font-sans leading-5">{TERMS_BODY}</pre>
          </ScrollArea>
          <label className="flex items-start gap-2 text-sm">
            ...
            />
            <span>
              [필수] 이용약관에 동의합니다. (회사는 통신판매중개자이며, 통신판매의 당사자가
              아닙니다.)
            </span>
          </label>
          <label className="flex items-start gap-2 text-sm">
          ...
          {isPartner && (
            <label className="flex items-start gap-2 text-sm">
              ...
              />
              <span>
                [필수] 파트너 이용약관에 동의합니다. (계약 당사자는 파트너사와 관리단이며,
                플랫폼은 이행을 보증하지 않습니다.)
              </span>
            </label>
          )}
          <Button className="w-full" disabled={!allAgreed || submitting} onClick={submit}>
            {submitting ? "기록 중..." : "동의하고 시작하기"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
